...
func TestGetLines(t *testing.T) {
	for _, file := range getTestFiles(t) {
		reader, err := NewReaderFromFilename(file, *styles.Get("native"), formatters.TTY16m, ReaderOptions{})
		if err != nil {
...
func testHighlightingLineCount(t *testing.T, filenameWithPath string) {
	...
	reader, err := NewReaderFromFilename(filenameWithPath, *styles.Get("native"), formatters.TTY16m, ReaderOptions{})
	assert.NilError(t, err)
	err = reader._wait()
	assert.NilError(t, err)
...
func TestStatusText(t *testing.T) {
	...
	// Test with filename
	testMe, err := NewReaderFromFilename(samplesDir+"/empty", *styles.Get("native"), formatters.TTY16m, ReaderOptions{})
	assert.NilError(t, err)
...
func testCompressedFile(t *testing.T, filename string) {
	...
	reader, e := NewReaderFromFilename(filenameWithPath, *styles.Get("native"), formatters.TTY16m, ReaderOptions{})
	if e != nil {
...
func TestReadFileDoneNoHighlighting(t *testing.T) {
	testMe, err := NewReaderFromFilename(samplesDir+"/empty",
		*styles.Get("Native"), formatters.TTY, ReaderOptions{})
	assert.NilError(t, err)
...
func TestReadStreamDoneNoHighlighting(t *testing.T) {
	testMe := NewReaderFromStream("", strings.NewReader("Johan"), chroma.Style{}, nil, ReaderOptions{})
...
func TestReadStreamDoneYesHighlighting(t *testing.T) {
	testMe := NewReaderFromStream("",
		...
		*styles.Get("Native"), formatters.TTY, ReaderOptions{Lexer: lexers.EmacsLisp})
...
func TestFormatJson(t *testing.T) {
	...
	testMe := NewReaderFromStream("JSON test", jsonStream, *styles.Get("Native"), formatters.TTY, ReaderOptions{})
...
func TestReadUpdatingFile(t *testing.T) {
	...
	testMe, err := NewReaderFromFilename(file.Name(), *styles.Get("native"), formatters.TTY16m, ReaderOptions{})
	assert.NilError(t, err)
...
func BenchmarkReaderDone(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		...
		readMe, err := NewReaderFromFilename(filename, *styles.Get("native"), formatters.TTY16m, ReaderOptions{})
...
func BenchmarkReadLargeFile(b *testing.B) {
	...
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		readMe, err := NewReaderFromFilename(largeFileName, *styles.Get("native"), formatters.TTY16m, ReaderOptions{})
		assert.NilError(b, err)
